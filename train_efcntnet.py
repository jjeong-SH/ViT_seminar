...
def train(args):
    ...
    if args.dataset == 'MNISTDataset':
        model = EfficientNet.from_name('efficientnet-b3', in_channels=1, num_classes=num_classes).to('cuda')
    else:
        model = EfficientNet.from_name('efficientnet-b3', in_channels=3, num_classes=num_classes).to('cuda')
    ...
    # -- logging
    log_dir = os.path.join('log/EfficientNet', args.dataset)
    ...
    pth_dir = os.path.join('efficientnet_pytorch', args.dataset)
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            if val_loss < best_val_loss:
                ...
                torch.save(model.state_dict(), f'{pth_dir}/_epoch_{epoch+1}_loss_{val_loss}.pth')
